...
public class Client implements Runnable, Constants {
    ...
    public Game readGame() throws IOException {
        ...
        if (getDataInputStream().readBoolean()) {
            ...
            // getMainFrame().getDrawClient().getGamerName( ( getNumberGamer() +
            // numberWinner) % names.length) + " winner!");
            return null;
        } else {
            // number gamer
...
//		readCountStonesGamers();
    ...
    private void setMessage(Game game) {
        if (getNumberGamer() == getActiveGamer()) {
            if (game.canStep()) getMainFrame().getDrawClient().setMessage(yourTurn);
            if (!game.canStep() && (getSizeBazar() != 0)) getMainFrame().getDrawClient().setMessage(takeFromMarket);
            if (!game.canStep() && (getSizeBazar() == 0)) getMainFrame().getDrawClient().setMessage(anotherPlayerTurn);
        }
        ;
        if (getNumberGamer() != getActiveGamer()) {
            getMainFrame().getDrawClient().setMessage(anotherPlayerTurn);
