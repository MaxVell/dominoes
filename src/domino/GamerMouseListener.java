...
public class GamerMouseListener implements MouseListener, MouseMotionListener {
    ...
    private void setStoneRectVisible(boolean visible) {
        if (getGame().getGameLine().canAttachStart(getStone()) || !visible) getStoneRect(0).setVisible(visible);
        if (getGame().getGameLine().canAttachEnd(getStone()) || !visible) getStoneRect(1).setVisible(visible);
    }
    ...
    public void mouseDragged(MouseEvent me) {
        // TODO Auto-generated method stub
        if (canChange()) getStoneView().setLocation(getX() + me.getX() - x, getY() + me.getY() - y);
    }
