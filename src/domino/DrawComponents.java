...
public class DrawComponents {
    ...
    private boolean nextStoneGameLineStart(StoneView preview, StoneView next, boolean isLeft) {
        ...
        if (isLeft) {
            if (preview.getLocation().x - 2 * next.getStoneWidth() > getGameLinePanelX()) {
                ...
                if (isTranfered) next.setTheta(next.getTheta() + Math.PI);
                ...
                // if(isTranfered)
                if (next.getStone().getFinish() != preview.getStartNumber(isLeft)) next.setTheta(next.getTheta() + Math.PI);
    ...
    private boolean nextStoneGameLineEnd(StoneView preview, StoneView next, boolean isLeft) {
        ...
        // boolean isTranfered = (down)?(next.getStone().getFinish() !=
        // preview.getEndNumber(isLeft)):(next.getStone().getStart() !=
        // preview.getEndNumber(isLeft));
        ...
        if (isLeft) {
            if (preview.getLocation().x - 2 * next.getStoneWidth() > getGameLinePanelX()) {
                ...
                next.setIsVertical(true);
                // if(!isTranfered)
                if (next.getStone().getStart() != preview.getEndNumber(isLeft)) next.setTheta(next.getTheta() + Math.PI);
    ...
    private void drawScore(Graphics2D g, String[] names, int[] scores, int countGamers) {
        ...
        g.fillRoundRect(posx, posy, 4 * getStoneWidth() + 2 * getIntProperty("insetsGamer") + getIntProperty("insetsStone"), 4 * getStoneWidth() + 2 * getIntProperty("insetsGamer") + getIntProperty("insetsStone"),
                getIntProperty("insetsString"), getIntProperty("insetsString"));
        g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
        g.setComposite(originalComposite);
        g.setColor(Color.BLACK);
    ...
    private String changeNameGamer(Graphics2D g, String nameGamer, int length) {
        ...
        if (count > 0) nameGamer = nameGamer.substring(0, nameGamer.length() - 3) + "...";
        return nameGamer;
    }
}
